...
impl FqElement {
    ...
    pub fn to_big(&self) -> Value<num_bigint::BigUint> {
        ...
        self.limbs[0]
            ...
            .map(|((l0, l1), l2)| fp_to_big(&l0) + fp_to_big(&l1) * &b + fp_to_big(&l2) * &b2)
    }
}
...
impl NonNativeFqChip {
            ...
            |mut region| {
                ...
                for i in 0..FQ_NUM_LIMBS {
                    ...
                    region.assign_advice(
                        ...
                        self.config.aux,
                        i,
                        || Value::known(prev_carry_fp),
                    )?;

                    let (c_i_val, carry_out): (Value<Fp>, Value<Fp>) = a.limbs[i]
                        .value
                        .zip(b.limbs[i].value)
                ...
                for i in 0..FQ_NUM_LIMBS {
                    ...
                    region.assign_fixed(
                        || format!("fq_{}", i),
                        self.config.fq_const,
                        row,
                        || Value::known(fq_limbs_fp[i]),
                    )?;

                    // Compute borrow_out = (r_i + k*fq_i + borrow_in - l_i) / 2^85
                    let borrow_out: Value<Fp> = result_limbs[i]
                        .value
                        .clone()
                        .zip(k_val)
                        ...
                        .map(|(((r, kv), bi), li)| {
                            let fq_i_big =
                                &fq_mod_big / &limb_base_big.pow(i as u32) % &limb_base_big;
    ...
    pub fn neg(
        ...
        layouter.assign_region(
            ...
            |mut region| {
                let zero_cell =
                    region.assign_advice(|| "zero", self.config.c, 0, || Value::known(Fp::ZERO))?;
    ...
    pub fn mul(
        ...
        let q_limb_vals: [Value<Fp>; 3] = array::from_fn(|i| {
            q_big
                .clone()
                .map(|q| big_to_fp(&(&q / &limb_base_big.pow(i as u32) % &limb_base_big)))
        });
        ...
        let qf_vals: [[Value<Fp>; 3]; 3] =
            array::from_fn(|i| array::from_fn(|j| q_limb_vals[i].map(|qv| qv * fq_limbs_fp[j])));

        // R limbs as BigUint
        let r_big_limbs: [Value<num_bigint::BigUint>; 3] =
            array::from_fn(|i| r_limb_vals[i].map(|v| fp_to_big(&v)));
        ...
        let p_sum_big: [Value<num_bigint::BigUint>; 5] = array::from_fn(|k| match k {
            ...
            1 => p_vals[0][1]
                .zip(p_vals[1][0])
                .map(|(x, y)| fp_to_big(&x) + fp_to_big(&y)),
            2 => p_vals[0][2]
                .zip(p_vals[1][1])
                .zip(p_vals[2][0])
        ...
        // Compute signed carries for the reduction chain. Lower convolution
        // positions can require a borrow from higher ones, so c_k is signed in
        // general. We witness c_k + 2^89 instead, which fits in the existing
        // 90-bit range check and keeps all advice values non-negative.
        let carry_offset_big = num_bigint::BigUint::from(1u64) << 89;
        let carry_offset_fp = big_to_fp(&carry_offset_big);
        let carry_const_first_fp = big_to_fp(&(&limb_base_big * &carry_offset_big));
        let carry_const_rest_fp = big_to_fp(&((&limb_base_big - 1u64) * &carry_offset_big));
        let carry_repr_fp: [Value<Fp>; 5] = {
            let mut carries = [Value::known(Fp::ZERO); 5];
            ...
            for k in 0..5 {
                ...
                let diff = p_sum_big[k]
                    ...
                    .zip(r_big_k)
                    .map(|((p, qf), r)| {
                        num_bigint::BigInt::from(p) + &prev_carry - num_bigint::BigInt::from(qf + r)
                    });
                ...
                if let Ok(diff_big) = diff.assign() {
                    ...
                    carries[k] = Value::known(big_to_fp(&carry_repr));
        ...
        Ok(layouter.assign_region(
            ...
            |mut region| {
                ...
                for k in 0..5 {
                    ...
                    // Offset-coded signed carry relation:
                    //   k=0: P_0 + B*O = (QF+R)_0 + B*(c_0 + O)
                    //   k>0: P_k + (c_{k-1}+O) + (B-1)O = (QF+R)_k + B*(c_k + O)
                    region.assign_advice(
                        || format!("cc{}_b", k),
                        self.config.b,
                        row,
    ...
    pub fn invert(
        ...
        layouter.assign_region(
            ...
            |mut region| {
                let one_cell =
                    region.assign_advice(|| "one", self.config.c, 0, || Value::known(Fp::ONE))?;
                let zero_cell =
                    region.assign_advice(|| "zero", self.config.c, 1, || Value::known(Fp::ZERO))?;
    ...
    /// Constrain two non-native Fq elements to be limb-wise equal.
    ...
    /// Decompose an Fq element into 255 little-endian bits.
    ///
    /// Each 85-bit limb is reconstructed independently and constrained equal
    /// to the original limb cell. If a limb has no existing cell, this method
    /// assigns a copy first and links the reconstruction to that copy.
    pub fn decompose_bits(
        ...
        layouter.assign_region(
            || "fq_decompose_bits",
            |mut region| {
                let mut out = Vec::with_capacity(FQ_NUM_LIMBS * FQ_LIMB_BITS);
                for limb_idx in 0..FQ_NUM_LIMBS {
                    let offset = limb_idx * (FQ_LIMB_BITS + 1);
                    let bits: Vec<Value<Fp>> = (0..FQ_LIMB_BITS)
                        .map(|i| {
                    ...
                    let recon_row = offset + FQ_LIMB_BITS;
