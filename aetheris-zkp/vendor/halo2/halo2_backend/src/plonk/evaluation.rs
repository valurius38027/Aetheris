...
impl<C: CurveAffine> Evaluator<C> {
    ...
    pub(in crate::plonk) fn evaluate_h(
        ...
        if crate::diagnostics::dbg_enabled() {
            eprintln!("[EVAL_H] y={:?} beta={:?} gamma={:?} theta={:?} extended_len={} n={} extended_k={} k={}",
                y, beta, gamma, theta, size, 1u64 << domain.k(), domain.extended_k(), domain.k());
        }
        ...
        if crate::diagnostics::dbg_enabled() {
            ...
            let num_zero = values.values.iter().filter(|v| **v == zero).count();
            let num_nonzero = values.values.len() - num_zero;
            if num_nonzero > 0 {
                eprintln!("[EVAL_H] f_coset: zero={} non_zero={} total={} first_nonzero={:?}", num_zero, num_nonzero, values.values.len(),
                    values.values.iter().find(|v| **v != zero).unwrap_or(&zero));
            } else {
                eprintln!("[EVAL_H] f_coset: ALL ZERO (f(X) is zero polynomial)");
            }
            eprintln!("[EVAL_H] f_coset[0] (zeta_eval)={:?}", values.values[0]);
        }
        values
    }
}
