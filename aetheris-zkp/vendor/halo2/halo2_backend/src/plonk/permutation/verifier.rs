        ...
        iter::empty()
            ...
            .chain(
                self.sets
                    .first()
                    .map(|first_set| {
                        let expr = l_0 * (C::Scalar::ONE - first_set.permutation_product_eval);
                        if crate::diagnostics::dbg_enabled() {
                            eprintln!("[PERM-VERIFIER] first_set_boundary l_0={:?} z_0={:?} expr={:?}", l_0, first_set.permutation_product_eval, expr);
                        }
                        expr
                    }),
            )
            ...
            .chain(self.sets.last().map(|last_set| {
                let expr = (last_set.permutation_product_eval.square() - last_set.permutation_product_eval) * l_last;
                if crate::diagnostics::dbg_enabled() {
                    eprintln!("[PERM-VERIFIER] last_set_boundary l_last={:?} z_last={:?} expr={:?}", l_last, last_set.permutation_product_eval, expr);
                }
                expr
            }))
            ...
            .chain(
                self.sets
                    ...
                    .map(move |(set, prev_last)| {
                        let expr = (set - prev_last) * l_0;
                        if crate::diagnostics::dbg_enabled() {
                            eprintln!("[PERM-VERIFIER] chain_constraint z_i={:?} z_prev_last={:?} expr={:?}", set, prev_last, expr);
                        }
                        expr
                    }),
            )
            ...
            .chain(
                self.sets
                    ...
                    .map(move |(chunk_index, ((set, columns), permutation_evals))| {
                        ...
                        for (col_idx, (eval, permutation_eval)) in columns
                            .iter()
                            ...
                            .enumerate()
                        {
                            let term = eval + (*beta * permutation_eval) + *gamma;
                            if crate::diagnostics::dbg_enabled() {
                                eprintln!("[PERM-VERIFIER] main_left chunk={} col={} eval={:?} perm_eval={:?} beta={:?} gamma={:?} term={:?}", chunk_index, col_idx, eval, permutation_eval, *beta, *gamma, term);
                            }
                            left *= term;
                        }
                        ...
                        for (col_idx, eval) in columns.iter().map(|&column| match column.column_type {
                            Any::Advice => {
                                advice_evals[vk.cs.get_any_query_index(column, Rotation::cur())]
                            }
                            Any::Fixed => {
                            ...
                            let term = eval + current_delta + *gamma;
                            if crate::diagnostics::dbg_enabled() {
                                eprintln!("[PERM-VERIFIER] main_right chunk={} col={} eval={:?} current_delta={:?} gamma={:?} term={:?}", chunk_index, col_idx, eval, current_delta, *gamma, term);
                            }
                            right *= term;
                            current_delta *= &C::Scalar::DELTA;
                        }

                        let active_row = C::Scalar::ONE - (l_last + l_blind);
                        let expr = (left - right) * active_row;
                        if crate::diagnostics::dbg_enabled() {
                            eprintln!("[PERM-VERIFIER] main_constraint chunk={} left={:?} right={:?} active_row={:?} expr={:?} z_next={:?} z_cur={:?}", chunk_index, left, right, active_row, expr, set.permutation_product_next_eval, set.permutation_product_eval);
                        }
                        expr
                    }),
            )
