...
pub struct ProverQuery<'com, C: CurveAffine> {
    ...
    /// Blinding factor used in the commitment to this polynomial.
    /// Required by the IPA prover to compute the initial cumulative blinding r'
    /// (which is then propagated through per-round blinds s_j, s'_j).
    /// The IPA verifier does NOT need this directly — it only needs the cumulative r',
    /// which the prover transmits after the final a_final.
