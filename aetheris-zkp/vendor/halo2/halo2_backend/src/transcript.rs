    ...
    fn common_point(&mut self, point: C) -> io::Result<()> {
        ...
        let coords: Coordinates<C> = Option::from(point.coordinates()).ok_or_else(|| {
            if crate::diagnostics::dbg_enabled() {
                eprintln!("[TRANSCRIPT-DBG-A] common_point(Blake2bWrite) point_at_infinity at {:?}", std::backtrace::Backtrace::capture());
            }
            io::Error::new(
                io::ErrorKind::Other,
    ...
    C::Scalar: FromUniformBytes<64>,
{
    ...
    fn common_point(&mut self, point: C) -> io::Result<()> {
        ...
        let coords: Coordinates<C> = Option::from(point.coordinates()).ok_or_else(|| {
            if crate::diagnostics::dbg_enabled() {
                eprintln!("[TRANSCRIPT-DBG-B] common_point(Keccak256Read) point_at_infinity at {:?}", std::backtrace::Backtrace::capture());
            }
            io::Error::new(
                io::ErrorKind::Other,
                "cannot write points at infinity to the transcript",
    ...
    C::Scalar: FromUniformBytes<64>,
{
    ...
    fn common_point(&mut self, point: C) -> io::Result<()> {
        ...
        let coords: Coordinates<C> = Option::from(point.coordinates()).ok_or_else(|| {
            if crate::diagnostics::dbg_enabled() {
                eprintln!("[TRANSCRIPT-DBG-C] common_point(Keccak256Write) point_at_infinity at {:?}", std::backtrace::Backtrace::capture());
            }
            io::Error::new(
                io::ErrorKind::Other,
                "cannot write points at infinity to the transcript",
