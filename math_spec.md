...
### 6.1 Fair Launch 与线性排放

Aetheris 不设创始人大额预分配，所有代币通过 VDF 挖矿线性排放产生。
...
| 参数 | 值 | 说明 |
|------|-----|------|
| $R_0$ | $10^8$ atoms (1 AET) | 初始区块奖励 |
| $N$ | $4.2 \times 10^7$ blocks | 排放总区块数 |
| $T_{block}$ | 10 sec | 目标出块时间 |
| 总供给 | $\frac{R_0 N}{2} = 2.1 \times 10^{15}$ atoms (21M AET) | 数学上限 |

### 6.2 交易手续费

手续费通过 `public_amount` 字段约束，100% 燃烧：

$$\text{fee} = \max(\text{MIN\_TX\_FEE},\ \text{tx\_size} \times \text{FEE\_PER\_BYTE})$$

- 最小手续费：$10^4$ atoms (0.0001 AET)
- 燃烧机制：$\text{coinbase} = \max(0,\ \text{reward}(h) - \sum \text{fees})$
- ZK 约束：$\text{total\_in} - \text{total\_out} - \text{fee} = 0$（复用现有守恒电路）

### 6.3 经济参数总表

| 参数 | 值 | 来源 |
|------|-----|------|
| $R_0$ | 100,000,000 atoms | 代码常数 |
| $N$ | 42,000,000 | 代码常数 |
| MIN_TX_FEE | 10,000 atoms | 协议常数 |
| 总供给上限 | 21,000,000 AET | 数学推导 |
...
### 7.1 日蚀抵抗定理 (Eclipse Resistance Theorems)

Aetheris 的安全性不依赖"诚实多数"假设。以下定理证明即使在完全网络隔离的极端条件下，协议仍提供可量化的数学保证。

**定理 7.1（主权完整性）**：设节点 $V$ 被攻击者完全日蚀，仅接收攻击者提供的链 $C_{adv}$。若 $C_{adv}$ 包含任意非法状态转换（如伪造交易、双花、价值不守恒），则 $V$ 的本地验证必将拒绝 $C_{adv}$。
...
**定理 7.2（日蚀可检测性）**：设 $V$ 自创世以来已计算 $n$ 个 VDF 迭代（输出序列 $y_0, y_1, ..., y_{n-1}$），但仅收到 $m < n$ 个对应区块。则 $V$ 数学上确定自己被日蚀攻击。
...
**定理 7.3（有限损害）**：日蚀攻击者对 $V$ 造成的损害在数学上有上界：
...
**定理 7.4（可恢复性）**：设 $V$ 在时刻 $t_0$ 被日蚀，在时刻 $t_1 > t_0$ 重新获得与诚实网络 $H$ 的连接。则 $V$ 可在无信任假设下确定规范链。
...
**定理 7.5（活性下界）**：设攻击者对 $V$ 的日蚀持续时间为 $\Delta t$。$V$ 在 $\Delta t$ 期间无法向诚实网络提交交易。此延迟是信息论必然——节点无法知道其未接收到的数据。但 $V$ 在此期间的资金安全性和状态完整性由定理 6.1 保证，且 $\Delta t$ 结束后 $V$ 可通过定理 6.4 恢复。
...
Aetheris 不假设攻击者追求经济利益最大化。定理 7.1-7.5 的证明仅依赖密码学假设（VDF 的序贯性、ZK 证明的可靠性、哈希函数的抗碰撞性），不涉及经济理性假设。

---

## 8. 集体递归证明与流式聚合 (Gossip-Aggregation)

为了实现原子化的平等并消除算力门槛，Aetheris 采用基于 **Halo2 Accumulation Scheme** 的集体递归证明方案。

### 8.1 原子证明 (Atomic Proof)
...
### 8.2 流式聚合函数 (Aggregation Function)
...
### 9.2 算力门槛消除定理 (Anti-Monopoly Theorem)
