...
*Last updated: 2026-06-13* (Stage 26: P0 ✅, P1 ✅, P2 ✅ C-5, Stage 26.5: ✅ D-2 Fee Burning)
...
| **C-1** | ✅ P1 | `9455f82` | Host-side commitment binding check + instance copy constraint. Cross-field (Fq vs Fp) prevents in-circuit gate. |
| C-2 | ⏳ P1 | — | Input membership + nullifier proof |
| **C-5** | ✅ P2 | (current) | Nullifier/commitment insertion before block DB write; intra-block double-spend detection; crash-safe write ordering |
...
| **P1** | 电路语义绑定 | ✅ H-5, ✅ C-4/L-2 (`4959c12`), ✅ C-1 (`9455f82`), ⏳ C-2 |
...
## Stage 26.5 — D-2 Fee Burning (手续费燃烧) (2026-06-13)

**Scope**: Implementation roadmap D-2 — 100% fee burning for spam prevention.

### Changes

| File | Change |
|------|--------|
| `aetheris-node/src/state.rs` | `validate_issuance_rules`: coinbase reward = `max(0, base_reward - total_fees)`; non-coinbase `public_amount>0` allowed (represents fee); block with fees but no coinbase rejected |
| `aetheris-node/src/main.rs` | Miner prepends coinbase TX (`public_amount = max(0, reward - total_fees)`) to all proposed blocks; `calculate_block_reward_atoms` / `ShieldedOutput` imported |

### Design Decisions
- **No circuit changes**: Fee burning reuses existing `public_amount` field. The ZK conservation circuit already ensures `sum(outputs) - sum(inputs) - public_amount == 0` — fees are deducted from the net value flow without additional constraints.
- **Coinbase required when fees present**: If non-coinbase transactions pay fees (`public_amount > 0`), the block MUST include a coinbase to receive the reduced reward. This prevents fee burning without miner compensation.
- **Phase 0 compatibility**: Blocks without coinbase and without fees still pass (no coinbase not required when no fees).
...
| aetheris-core 25/25 | ✅ |
| aetheris-node 11 + 2 + 2 | ✅ |
| aetheris-zkp 124/124 | ✅ |
| FFI (pre-existing crash only) | ✅ |
